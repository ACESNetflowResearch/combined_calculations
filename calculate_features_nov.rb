...
begin
    ...
    all_src_ips = con.query("SELECT DISTINCT ip FROM ip_labels_nov_4 WHERE label = 1") #get all the distinct src IPS
    ...
    all_src_ips.each do |ip|
	   ...
	   ip_data = con.query("SELECT * FROM netflow_nov2 WHERE src_IP = '#{ip[0]}'")
	   
	   #ip_label = con.query("SELECT label FROM ip_labels_2 WHERE ip = '#{ip[0]}'")
	   #ip_label.each do |l|
	#	  label = l[0]
	 #  end
	   ip_data.each do |flow|
		#  label = con.query("SELECT label FROM ip_labels_2 WHERE ip = 'flow[3]'")
		  #puts "here is the label: #{label}"
		  c.total_flows += 1
